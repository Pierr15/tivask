...
export type TicketRow={id:string;conversationId:string;whatsappNumber:string;name:string|null;question:string;summary:string;summarySource:string;reason:string;priority:Priority;status:TicketStatus;createdAt:string;updatedAt:string;resolvedAt:string|null};
...
export class Persistence {
 ...
 let remote:TicketRow[]=[];try {remote=(await db(()=>prisma.ticket.findMany({orderBy:{createdAt:'desc'},take:500}))).map(t=>({...t,createdAt:t.createdAt.toISOString(),updatedAt:t.updatedAt.toISOString(),resolvedAt:t.resolvedAt?.toISOString()??null}));}catch{}
 const merged=new Map([...remote,...(await this.ticketStore.read()).tickets.map(plain)].map(t=>[t.id,t]));return [...merged.values()].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
 }
 ...
 for(const m of s.messages.filter(r=>r.pending)){const v=plain(m);await db(()=>prisma.message.upsert({where:{id:v.id},create:v,update:{delivery:v.delivery}}));m.pending=false;}
 ...
 // Retain recent messages for outage memory; unsynced rows are never discarded.
 ...
 async pending(){const s=await this.journal.read();const t=await this.ticketStore.read();return [...s.conversations,...s.messages,...s.events,...t.tickets].filter(r=>r.pending).length;}
