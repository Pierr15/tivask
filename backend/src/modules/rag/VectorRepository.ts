...
export class VectorRepository {
  ...
  async saveVector(id: string, vector: number[]) {
    try {
      await db(
        () =>
          prisma.$executeRaw`UPDATE "DocumentChunk" SET vector = ${JSON.stringify(vector)}::vector WHERE id = ${id}`,
      );
      this.mode = "PGVECTOR";
    } catch {
 ...
 ORDER BY c.vector <=> ${JSON.stringify(vector)}::vector LIMIT ${safeLimit}`,
      );
      this.mode = "PGVECTOR";
      return rows.map((r) => ({ ...r, source: r.filename }));
    } catch {
      this.mode = "POSTGRES_JSON_COSINE";
      const rows = await db(() =>
        prisma.documentChunk.findMany({
          ...
          include: { document: { select: { demo: true } } },
          take: 2000,
